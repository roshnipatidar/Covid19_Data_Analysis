...
case 
	when DAY(date) BETWEEN 1 AND 7 THEN 'Week_1'
	when DAY(date) BETWEEN 8 AND 14 THEN 'Week_2'
	when DAY(date) BETWEEN 15 AND 21 THEN 'Week_3'
	when DAY(date) BETWEEN 22 AND 28 THEN 'Week_4'
	ELSE 'Week_5'
	END AS Weeks,
...
GROUP BY year_,month_name,month_,Weeks
ORDER BY year_,month_,Weeks
)
SELECT year_,month_name,month_,Weeks AS week_number,total_confirmed,total_recovered,total_tests,total_death
FROM cte
ORDER BY year_,month_,Weeks;
    
-- Monthly evolution of number of confirmed cases, recovered cases, deaths, tests. 
    with cte as(
select year(date) as year_, 
monthname(date) as month_name,
month(date) as month_, 
...
with cte1 as 
(
select state_code, district_code, tested_total, deceased_total,population_meta,
(tested_total/population_meta) as test_ratio  
...
-- analysis of number of deaths across all state and district wise in all category 
with cte1 as 
(
select state_code,district_code, tested_total, deceased_total,population_meta,
(tested_total/population_meta) as test_ratio  
...
select state_code,district_code,Category,
sum(deceased_total) as TotalDeaths,
(sum(deceased_total) * 100.0) / sum(sum(deceased_total)) over () as PercentageOfTotalDeaths
from cte2
where state_code = 'BR'
group by  state_code, district_code,Category
order by state_code,district_code,  Category;
...
with cte as 
(
select state_code,district_code, confirmed_delta7 , population_meta ,(vaccinated1_delta7 + vaccinated2_delta7) as Vaccinated 
from district_covid_data 
...
select statecode, statepopulation,statetotaltested,
(StateTotalVaccinated1+StateTotalVaccinated2) as total_vaccinated
from state_covid_data group by 1,2,3;
